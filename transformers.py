...
class NumericalImputer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X, y=None):
        X_c = X.copy()
        for col in ['Age', 'Weight', 'Heart_Rate', 'Body_Temperature_°C']:
            if col == 'Age':
                ...
                if X_c[col].isna().sum() > 0:
                    X_c.fillna({f'{col}': X_c['Animal_Type'].map(func=lambda x: np.round(self.animals_weight[x], decimals=1) if x else 30)}, axis=0, inplace=True)
                    continue
                else:
                    continue
            else:
